...
```{r}
tweets_CUSO <- search_tweets("CUSO", n = 1000)
```
...
```{r}
head(tweets_CUSO$text)
plot(tweets_CUSO$created_at)
```

On peut faire mieux avec d'autres packages, comme `ggplot2`

```{r}
#install.packages("ggplot2")
library(ggplot2)
nouv_graph<-ggplot(tweets_CUSO, aes(x = created_at)) + geom_histogram()
nouv_graph
```
...
meilleur_graph<-ggplot(tweets_CUSO, aes(x = created_at)) + geom_histogram(binwidth = 5000) +
